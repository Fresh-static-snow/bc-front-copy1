import { Meta, StoryFn, StoryObj } from '@storybook/react';
import { useState } from 'react';

import { TournamentItemMobile } from '@/entities/calendar';
import { DisciplineTitleMobile } from '@/entities/calendar/ui/DisciplineTitle/DisciplineTitle.mobile';
import { TournamentInCalendarEntity } from '@/shared/types/entities.types';

export default {
  title: 'entities/calendar/TournamentItemMobile',
  component: TournamentItemMobile,
  tags: ['autodocs'],
  argTypes: {
    tournament: { control: { type: null } },
    onClickTournament: { control: { type: null } },
    filters: { control: { type: null } },
    onClickMatch: { control: { type: null } },
    discipline: { control: { type: null } },
    index: { control: { type: null } },
    isVisibleDisciplineTitle: { control: { type: null } },
    setDisciplineTitle: { control: { type: null } },
  },
} as Meta<typeof TournamentItemMobile>;

type Story = StoryObj<typeof TournamentItemMobile>;
type StoryTemplate = StoryFn<typeof TournamentItemMobile>;

const tournament: TournamentInCalendarEntity = {
  id: 1,
  tier: 1,
  title: 'The International 2019',
  type: { id: 1, name: 'Homecast' },
  entity_type: 'tournament',
  main_participants: [
    {
      id: 1,
      avatar: { url: 'https://picsum.photos/200' },
      display_name: 'Karl Lagerfeld',
      nick: 'Some_Nick',
      first_name: 'Karl',
      last_name: 'Lagerfeld',
    },
  ],
  media_representatives: [
    {
      id: 2,
      avatar: { url: '' },
      display_name: 'Karl Lagerfeld',
      nick: 'Some_Nick',
      first_name: 'Karl',
      last_name: 'Lagerfeld',
    },
  ],
  visible: true,
  matches: [
    {
      id: 1,
      start_time: '14:00',
      end_time: '15:00',
      start_date: '2021-08-10',
      visible: false,
      format: 'BO3',
      team_one: 'Thunder Awaken',
      team_two: 'OG',
      type: 'Match',
      match_casts: [],
    },
    {
      id: 2,
      start_time: '14:00',
      end_time: '15:00',
      start_date: '2021-08-10',
      visible: true,
      format: 'BO3',
      team_one: 'Thunder Awaken',
      team_two: 'OG',
      type: 'Match',
      match_casts: [
        {
          id: 1,
          language: {
            id: 1,
            name: 'EN',
            keyword: 'en',
          },
          analytics: [
            {
              id: 1,
              display_name: 'Graham Connelly',
              nick: 'Some_Nick',
              first_name: 'Graham',
              last_name: 'Connelly',
              avatar: { url: 'https://picsum.photos/200' },
            },
          ],
          channels: [
            {
              id: 1,
              name: 'fortnite_mc_ua',
            },
          ],
          commentators: [
            {
              id: 2,
              display_name: 'Graham Connelly',
              nick: 'Some_Nick',
              first_name: 'Graham',
              last_name: 'Connelly',
              avatar: { url: '' },
            },
          ],
          host_analytic: {
            id: 3,
            display_name: 'Graham Connelly',
            nick: 'Some_Nick',
            first_name: 'Graham',
            last_name: 'Connelly',
            avatar: { url: '' },
          },
          backup_commentators: [
            {
              id: 3,
              display_name: 'Graham Connelly',
              nick: 'Some_Nick',
              first_name: 'Graham',
              last_name: 'Connelly',
              avatar: { url: '' },
            },
          ],
          staff_members: [
            {
              id: 3,
              display_name: 'Graham Connelly',
              nick: 'Some_Nick',
              first_name: 'Graham',
              last_name: 'Connelly',
              avatar: { url: '' },
            },
          ],
          analytic_studio: {
            id: 1,
            name: 'Cast 9.1',
            keyword: 'cast_9_1',
          },
          studio: {
            id: 1,
            name: 'Cast 9.2',
            keyword: 'cast_9_2',
          },
          setup: {
            id: 1,
            name: 'Setup 1',
          },
          stream: {
            id: 1,
            name: 'Stream 1',
          },
        },
      ],
    },
  ],
  ui_template: {
    primary: '#FF0000',
  },
  start_date: '2021-01-01',
  end_date: '2021-01-02',
  analytics: [],
  commentators: [],
  discipline_keyword: 'dota2',
};

const Template: StoryTemplate = (args) => {
  const [disciplineTitle, setDisciplineTitle] = useState<string>('---');

  return (
    <div
      style={{
        position: 'relative',
        display: 'flex',
        flexDirection: 'column',
        padding: '68px 0 0',
      }}
    >
      <DisciplineTitleMobile title={disciplineTitle} />

      <TournamentItemMobile
        tournament={tournament}
        {...args}
        setDisciplineTitle={setDisciplineTitle}
      />
    </div>
  );
};

export const Simple: Story = {
  render: Template,
  parameters: {
    docs: {
      source: {
        code: `
<div
  style={{
    position: 'relative',
    display: 'flex',
    flexDirection: 'column',
    padding: '68px 0 0',
  }}
>
  <DisciplineTitleMobile title={disciplineTitle} />

  <TournamentItemMobile
    tournament={tournament}
    setDisciplineTitle={setDisciplineTitle}
  />
</div>
        `,
      },
    },
  },
};

export const WithFilters: Story = {
  render: Template,
  args: {
    filters: {
      staff_members: ['3'],
      channel: ['1'],
      commentators: ['2'],
    },
  },
  parameters: {
    docs: {
      source: {
        code: `
<div
  style={{
    position: 'relative',
    display: 'flex',
    flexDirection: 'column',
    padding: '68px 0 0',
  }}
>
  <DisciplineTitleMobile title={disciplineTitle} />

  <TournamentItemMobile
    tournament={tournament}
    setDisciplineTitle={setDisciplineTitle}
    filters={
      staff_members: ['3'],
      channel: ['1'],
      commentators: ['2'],
    }
  />
</div>
        `,
      },
    },
  },
};
