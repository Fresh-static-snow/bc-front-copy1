...
import { OptionWithInputValue } from '../../types';
import { CustomOption } from '../CustomOption/CustomOption';
import { CustomPopup } from '../CustomPopup/CustomPopup';
import { CustomTextField } from '../CustomTextField/CustomTextField';
import { PopupIcon } from '../PopupIcon/PopupIcon';
import * as S from './AutocompleteSingle.styles';
import { AutocompleteSingleProps } from './AutocompleteSingle.types';

const filter = createFilterOptions<OptionWithInputValue>();

/**
 * The component is used to select one option from a list.
 *
 * Component is intended to work only in conjunction with the React Hook Form.
 */
export const AutocompleteSingle: React.FC<AutocompleteSingleProps> = ({
  options,
  name,
  control,
  disabled,
  ...
  const onRenderInput = useCallback(
    (params: AutocompleteRenderInputParams) => (
      <CustomTextField
        ...
        option={field.value as PrimarySelectableValue}
        isLoading={isLoading}
      />
    ),
    [AdditionalElement, field.value, isLoading, placeholder],
  );
  ...
  useDeepCompareEffect(() => {
    if (options) {
      const currentValue = field?.value as PrimarySelectableValue;
      const updatedState =
        options?.find((option) => option.value === currentValue?.value) ?? currentValue;
  ...
  return (
    <S.AutocompleteSingle data-testid="AutocompleteSingle">
      <S.Autocomplete
        disabled={disabled}
        loading={isLoading}
