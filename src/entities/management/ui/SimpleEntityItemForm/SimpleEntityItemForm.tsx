...
import { simpleEntityItemDefaultValues } from './SimpleEntityItemForm.const';
import { simpleEntityItemFormSchema } from './SimpleEntityItemForm.schema';
import * as S from './SimpleEntityItemForm.styles';
import {
  SimpleEntityItemFormProps,
  SimpleEntityItemFormSchema,
} from './SimpleEntityItemForm.types';

export const SimpleEntityItemForm: React.FC<SimpleEntityItemFormProps> = ({
  ...
  const {
    ...
    reset,
    watch,
  } = useForm<SimpleEntityItemFormSchema>({
    resolver: yupResolver(simpleEntityItemFormSchema),
    mode: 'onSubmit',
    reValidateMode: 'onSubmit',
    defaultValues: { ...simpleEntityItemDefaultValues, ...defaultFormData },
  });
  const formDataValues = watch();

  const onSubmit: SubmitHandler<SimpleEntityItemFormSchema> = useCallback(
    async (data) => {
      await onSendData(data);

      // * If the user wants to add another entity, we don't close the form and reset it.
      if (!data.anotherOne) {
        reset(simpleEntityItemDefaultValues);
        onCloseModal();
      } else {
        reset({
          ...simpleEntityItemDefaultValues,
          anotherOne: true,
        });
      }
    },
